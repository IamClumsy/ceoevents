...
const schemes: Record<
  ColorScheme,
  { card: string; title: string; total: string; check: string; catHeader: string; subtotal: string }
> = {
  violet: {
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-800/60">
                    {cat.tasks.map((task, ti) => {
                      const state = taskStates[ci][ti];
                      const score = state.included
                        ? calcScore(task.task, task.points, state.used)
                        : 0;
                      return (
                        <tr
                          key={task.task}
